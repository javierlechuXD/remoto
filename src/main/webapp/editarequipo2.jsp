...
<body class="is-preload">
		...
		ArrayList<Liga> ligas = controladorBD.todasLigas();
		Equipo equipoEditar = controladorBD.dameEquipo(Integer.parseInt(request.getParameter("cod_equipo")));
		%>
			<!-- Header -->
				<div id="header">
					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								<h3>Editar Equipo</h3>
								<form action="Operaciones.jsp?tipo=editar_equipo&cod_equipo=<%=equipoEditar.getCod_equipo()%>" method="post">
									<div class="row gtr-50">
										<div class="">
											<input type="text" name="nombre" id="nombre" placeholder="Nombre del equipo" value="<%=equipoEditar.getNombre()%>" maxlength="15" required/>
										</div>
									<div class="">
										<select id="cod_liga" name="cod_liga" is="ms-dropdown" required style="width:450px" data-child-height="415" required>
											<option value="">Seleccione una Liga</option>
											<% for(int i = 0; i<ligas.size();i++){%>
											<option value="<%=ligas.get(i).getCod_liga()%>" data-image="./images/ligas/<%=ligas.get(i).getCod_liga()%>.png" <%if(ligas.get(i).getCod_liga() == equipoEditar.getCod_liga()){ out.print("selected='selected'");}%>><%=ligas.get(i).getNombre()%></option>
											<%}%>
										</select>
									</div>
