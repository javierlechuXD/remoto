...
<body class="is-preload">
		...
		Jugador jugadorEditar = controladorBD.dameJugador(Integer.parseInt(request.getParameter("cod_jugador")));%>
			<!-- Header -->
				<div id="header">
					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								<form action="Operaciones.jsp?tipo=editar_jugador&cod_jugador=<%=jugadorEditar.getCod_jugador()%>" method="post">
									<div class="row gtr-50">
										<div class="">
											<input type="text" name="nombre" id="nombre" placeholder="Nombre del jugador" value="<%=jugadorEditar.getNombre()%>" maxlength="9" required/>
										</div>
										<div class="">
										
											<select id="cod_equipo" name="cod_equipo" is="ms-dropdown" required style="width:450px" data-child-height="415" required>
												<option value="">Seleccione un Equipo</option>
												<% for(int i = 0; i<equipos.size();i++){%>
												<option value="<%=equipos.get(i).getCod_equipo()%>" data-image="./images/equipos/<%=equipos.get(i).getCod_equipo()%>.png" <%if(equipos.get(i).getCod_equipo() == jugadorEditar.getCod_equipo()){ out.print("selected='selected'");}%>><%=equipos.get(i).getNombre()%></option>
												
												<%}%>
											</select>
										</div>
										<div class="">
											<input type="text" name="pierna" id="pierna" placeholder="Pierna del jugador" value="<%=jugadorEditar.getPierna()%>" maxlength="20" required/>
										</div>
										<div class="">
											<input type="number" name="altura" id="altura" placeholder="Altura del jugador" value="<%=jugadorEditar.getAltura()%>" maxlength = "4" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" required/>
										</div>
										<div class="">
											<input type="text" name="pais" id="pais" placeholder="País del jugador" value="<%=jugadorEditar.getPais()%>" maxlength="10" required/>
										</div>
									...
									<div style="padding-top:22px;">
											<ul class="actions">
												<li><input type="submit" name="submit" id="submit" value="EDITAR"></li>
											</ul>
									</div>
